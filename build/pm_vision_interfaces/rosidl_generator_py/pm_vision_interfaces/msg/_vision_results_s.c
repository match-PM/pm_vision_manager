...
bool pm_vision_interfaces__msg__vision_results__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // saved_images
    PyObject * field = PyObject_GetAttrString(_pymsg, "saved_images");
    if (!field) {
      return false;
    }
    {
      PyObject * seq_field = PySequence_Fast(field, "expected a sequence in 'saved_images'");
      if (!seq_field) {
        Py_DECREF(field);
        return false;
      }
      ...
      rosidl_runtime_c__String * dest = ros_message->saved_images.data;
      for (Py_ssize_t i = 0; i < size; ++i) {
...
PyObject * pm_vision_interfaces__msg__vision_results__convert_to_py(void * raw_ros_message)
{
  ...
  {  // saved_images
    ...
    rosidl_runtime_c__String * src = ros_message->saved_images.data;
    field = PyList_New(size);
    if (!field) {
      return NULL;
    }
    ...
    assert(PySequence_Check(field));
    {
      int rc = PyObject_SetAttrString(_pymessage, "saved_images", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
