...
class VisionProcessClass():
  ...
  def __init__(self, 
    ...
    self.path_config_path = package_share_directory + '/vision_assistant_path_config.yaml'
    ...
    self.vision_node.get_logger().info('Screen resolution: '+ str(self.screen_width)+'x'+str(self.screen_height))
  ...
  def apply_image_for_vizualization(self, window_name, image, display_time):
    ...
    if not found_tuples:
      self.vision_node.image_list.append((window_name, image, display_time, image_display_start_time))
      ...
      try:
        ...
        for image_in_folder in os.listdir(self.process_db_path):
          if (image_in_folder.endswith(".png")):
            ...
            if ((not self.VisionOK and self.show_image_on_error) or self.step_through_images) and self.launch_as_assistant and self.show_cross_validation_images: 
              self.next_image=False
  ...
  def load_path_config(self):
      try:
        ...
        self.vision_node.get_logger().error('Error opening vision assistant path configuration: ' + str(self.path_config_path)+ "!")
  ...
  def load_assistant_config(self):
    try:
      ...
      self.vision_node.get_logger().error('Error opening vision assistant configuration: ' + str(self.vision_assistant_config)+ "!")
  ...
  def set_display_time_for_exit(self):  # this function is used when the vision assistant instance is ended. 
  ...
  def load_camera_config(self):
    try:
      ...
      try:
        ...
        self.vision_node.get_logger().warn('Error in configuring camera or no exposure time interface available!')
      ...
      print(e)
      self.vision_node.get_logger().error('Error opening camera config file: ' + str(self.camera_config_path+self.camera_config_filename)+ "!")
      ...
      if (exposure_value < self.camera_exposure_time_max_val) and (exposure_value > self.camera_exposure_time_min_val):
        if (self.camera_exposure_time_set_value != exposure_value):
          ...
          if not client_test.wait_for_service(timeout_sec=2.0):
              self.vision_node.get_logger().error('Spawn Service not available')
  ...
  def load_process_file_metadata(self):
    try:
      ...
      self.vision_node.get_logger().error('Error opening process file: ' + str(self.process_file_path)+ "!")
