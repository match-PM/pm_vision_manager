...
class VisionAssistantApp(QMainWindow):
    ...
    def open_process_file(self):
        ...
        if file_path:
            ...
            success = (
                self.current_vision_pipeline.set_vision_pipeline_from_process_json(
                    file_path
                )
            )
    ...
    def set_function_states(self):
        for index in range(self.checkbox_list.count()):
            ...
            if check_state == Qt.CheckState.Checked:
                self.current_vision_pipeline.vision_functions[
                    index
                ].set_function_check_state(True)
            elif check_state == Qt.CheckState.Unchecked:
                self.current_vision_pipeline.vision_functions[
                    index
                ].set_function_check_state(False)
            else:
    ...
    def create_new_file(self):
        try:
            ...
            if (
                not self.save_as
                and self.current_vision_pipeline.process_name is not None
            ):
                self.current_vision_pipeline.vision_functions.clear()

            if file_name:
                self.current_vision_pipeline.process_name = os.path.basename(
    ...
    def set_vision_ui_from_pipeline(self):
        ...
        for function in self.current_vision_pipeline.vision_functions:
            function_checkbox = ReorderableCheckBoxListItem(
                function.vision_function_name
            )
            function_checkbox.setCheckState(
                Qt.CheckState.Checked
                if function.get_function_check_state()
    ...
    def delete_function_from_pipeline(self):
        ...
        if selected_function:
            ...
            self.current_vision_pipeline.remove_function_by_index(
                index_selected_function
            )
    ...
    def create_function_parameters_layout(self):
        ...
        if selected_function_name is not None:
            ...
            selected_function = self.current_vision_pipeline.return_function_by_index(
                function_position_in_pipeline
            )
            ...
            if selected_function:
                ...
                for param in selected_function.bool_params_list:
                    if param.param_name != "active":
                        ...
                        bool_widget.stateChanged.connect(
                            partial(self.bool_checkbox_cbk, bool_widget, param)
                        )
                ...
                for param in selected_function.string_list:
                    ...
                    string_input.textChanged.connect(
                        partial(self.stringBox_cbk, string_input, param)
                    )
                ...
                for param in selected_function.int_list:
                    ...
                    int_spin_box.setMinimum(N_INF_INT)
                    int_spin_box.setMaximum(INF_INT)
                    ...
                    print(param.max_val)
                    int_spin_box.setValue(
                        param.get_value()
                    )  # Set the initial value of the slider
                    int_spin_box.setReadOnly(False)  # Make the spin box non-editable
                    int_spin_box.setSingleStep(1)
                    int_spin_box.valueChanged.connect(
                        partial(self.intspinBox_cbk, int_spin_box, param)
                    )
                ...
                for param in selected_function.float_list:
                    ...
                    double_spinbox.setMinimum(N_INF_FLOAT)
                    double_spinbox.setMaximum(INF_FLOAT)
                    ...
                    double_spinbox.valueChanged.connect(
                        partial(self.float_box_cbk, double_spinbox, param)
                    )
                ...
                for param in selected_function.unsigned_int_params_list:
                    ...
                    unsigned_int_spin_box.setMinimum(0)
                    unsigned_int_spin_box.setMaximum(INF_INT)
                    ...
                    unsigned_int_spin_box.setValue(
                        param.get_value()
                    )  # Set the initial value of the slider
                    unsigned_int_spin_box.setReadOnly(
                        False
                    )  # Make the spin box non-editable
                    unsigned_int_spin_box.setSingleStep(1)
                    unsigned_int_spin_box.valueChanged.connect(
                        partial(
                            self.unsigned_intspinBox_cbk, unsigned_int_spin_box, param
                        )
                    )
                ...
                for param in selected_function.kernel_list:
                    ...
                    kernel_spin_box.setMinimum(0)
                    kernel_spin_box.setMaximum(INF_INT)

                    if isinstance(param.max_val, float):
                        kernel_spin_box.setMaximum(param.max_val)

                    kernel_spin_box.setValue(
                        param.get_value()
                    )  # Set the initial value of the slider
                    kernel_spin_box.setReadOnly(False)  # Make the spin box non-editable
                    kernel_spin_box.setSingleStep(2)
                    kernel_spin_box.valueChanged.connect(
                        partial(self.kernelspinBox_cbk, kernel_spin_box, param)
                    )
                ...
                for param in selected_function.list_param_list:
                    ...
                    list_param_box.activated.connect(
                        partial(self.param_list_cbk, list_param_box, param)
                    )
    ...
    def on_drop(self):
        self.current_vision_pipeline.move_function_to_indice(
            old_index=window.checkbox_list.drag_source_position,
    ...
    def initializeAppPaths(self):
        try:
            vision_manager_share_directory = get_package_share_directory(
                "pm_vision_manager"
            )
            vision_manager_path_config_path = (
                vision_manager_share_directory + "/vision_assistant_path_config.yaml"
            )
